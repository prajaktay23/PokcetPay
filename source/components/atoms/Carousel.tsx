...
//And TypeScript won’t complain — it knows .svg is a component. refer declaration.d.ts file and metro config file for tranformation
    ...
    <Swiper
      ...
      {data.map(item => (
        <View style={styles.slide} key={item.id}>
          {item.image}
          <PrimaryText style={{color: Colors.primaryText, fontSize: 12 , marginTop: '3%'}}>{item.text}</PrimaryText>
        </View>
      ))}
    </Swiper>
  );
